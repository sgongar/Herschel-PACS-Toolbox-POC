...
Purpose:
  ...
  that you check the effect of the pointing correction on your target, e.g. by 
  comparing the results of this script with those of the background 
  normalisation pipeline script for range scans.
  Please also note that this pipeline is currently not completely documented in
  the PACS Data Reduction Guide.
...
# When saveOutput is True, nameBasis will be used as basis for the filenames of 
# all outputs
saveOutput = False
...
nameBasis  = str(obsids.keys()[j]) + "_" + target + "_" + od + "_Hipe_" +\
             hipeVersion + "_calSet_" + calSet + "_" + camera + "_norm_poc"
...
# Extend the Status of Frames with the grating and chopper parameters GRATSCAN,
# CHOPPER, CHOPPOS used cal file: ChopperThrowDescription
...
if verbose:
    ppoint = slicedPlotPointing(slicedFrames, plotBoresight=False)
...
slicedFrames = specCorrectHerschelVelocity(slicedFrames,
                                           ...
                                           obs.auxiliary.pointing,
                                           obs.auxiliary.timeCorrelation,
                                           obs.auxiliary.horizons, force=True)
...
# Uses the high resolution Dec/Mec header and the cal files ChopperAngle and 
# ChopperJitterThreshold 
...
# Uses the high resolution Dec/Mec header and the cal file 
...
# Convert the signal level to the reference integration capacitance (which is
# the smallest one) used cal file: capacitanceRatios
...
# The calibration block is cut out of the slicedFrames, so only the scientific
# slices remain.
...
# Compute the telescope background flux from the off-source data, and normalise 
# the on-source data by this
# For more detail on the parameters, see the URM for this task
# The task uses the offRatio calibration product
slicedFrames, background = specRespCalToTelescope(slicedFrames, 
                                                  obs.auxiliary.hk,
                                                  calTree=calTree,
                                                  reduceNoise=1)
...
if verbose:
    ...
    pBack = PlotXY(background.refs[0].product.wavelengths,
                   background.refs[0].product.fluxJy,
                   xtitle="Wavelength [$\mu$m]", ytitle="Flux [Jy]",
                   titleText="Telescope Background")
...
slicedFrames = activateMasks(slicedFrames,
                             String1d(["SATURATION", "RAWSATURATION",
                                       "NOISYPIXELS", "BADPIXELS",
                                       "UNCLEANCHOP", "GRATMOVE",
                                       "GLITCH"]), exclusive=True)

# --- POINTING OFFSET CORRECTION

# The next tasks will determine the pointing, and compute a flux correction wrt 
# the pointing error. The correction is based on the assumption of (hence only 
# valid for) a point source.
# The instantaneous pointing can be estimated from the flux distribution between
# the central spaxels, compared to the flux distribution of a perfectly pointed
# point source (derived from the beam calfiles).
# This is the path followed when usePointingProduct = False.
# This relies entirely on the flux measured in the central 9 spaxels, hence it
# requires sufficient S/N.

# The pointing error can also be seen as made of two components: an absolute 
# offset, and a jitter around it.
# The absolute offset can only be determined thanks to the tasks described 
# above, but one can determine an median offset per pointing, i.e. per nod.
# That allows to determine the absolute offset from coadded data, and thus have 
# a better result for weaker sources.
# This is the path followed when usePointingProduct = True
# The jitter is then derived from the gyro-propagated pointing products that 
# come with the observation.
# NB: these products have been improved by new calculations (gyro-propagation), 
# so make sure your level0 products have been processed with SPG 11 or higher
# (see the PACS and Herschel wiki)
...
if saveOutput:
    saveSlicedCopy(slicedFrames, nameBasis + "_slicedFrames_prePOC",
                   poolLocation=outputDir)

# STEP 1
# Determine a map that is the chi-squared of the differences between the flux
# of your point source and the flux of the centred beam
# Task uses the various beams calibration products 
# Some parameters:
#   oversample: oversampling factor to interpolate the beams over
#       The higher the oversample, the more precise the position determination
#       will be the requested computer resources are proportional to
#       oversample^2.
#   smoothFactor: smoothing factor along the spectral domain (working in 
#       grating positions rather than wavelength)
#       Every grating plateau (set of data-points taken at the same grating
#       position) is medianed by default. 
#       Or you can median every "smoothFactor" grating plateau instead, rather
#       than just one: leading to a higher S/N
#       When perNod is True, smoothFactor is ignored and a smoothFactor is
#       calculated in a way that around 5 grating plateaux will be left. 
oversampleBeam = 5
smoothFactor1 = 10
chiSqr = specDetermineChiSquare(slicedFrames, calTree=calTree, spaxels=spaxels,
                                oversample=oversampleBeam,
                                smoothFactor=smoothFactor1,
                                perNod=usePointingProduct)

# STEP 2
# Using the chiSqr, now determine the pointing offsets (i.e. those which caused 
# the measured flux offsets)
# Some parameters:
#     chiThreshold: defines the minimum area of the chi square map: MIN(Chi2) 
#                   < chi2 < chi_threshold*MIN(Chi2)
#     smoothWidth: median box width for smoothing the signal along the timeline:
#                  default is 0, i.e. no smoothing
# The output product contains the derived source position, stored in the array
# coordinates of the beam calibration products (25x25 images with a step of 2.5
# arcsec)
pointingOffset = specDeterminePointingOffset(slicedFrames, chiSqr,
                                             spaxels=spaxels,
                                             chiThreshold=1.2, smoothWidth=0,
                                             sigmaClip=2.5, searchMax=1,
                                             useMedianPointingInLeak=1)

# STEP 3
# For slicedFrames that have Ra/Dec arrays calculated using the gyro-propagated 
# pointing product the following task creates a pointing offset
# By default, we get the median pointingOffset from steps 1 & 2 above, and the
# jitter from the spacecraft's pointing product (here)
# Alternatively, you may also skip steps 1 & 2, and directly plug any
# pre-determined pointingOffset here (e.g. derived with another method, or from 
# the other camera)
...
# STEP 4 
# Calculate the flux correction based on the pointing offset determined in the
# previous step and add the flux correction factors to the pointingOffset
# product.
if usePointingProduct:
    pointingOffset = specDeterminePointingOffsetFromPreCalculatedPointing(slicedFrames,
                                                                          pointingOffset,
                                                                          calTree=calTree,
                                                                          spaxels=spaxels)
...
# Apply a small time- and wavelength-dependent correction to the telescope
# background to recover the abs. flux calibration
...
if saveOutput: 
    saveSlicedCopy(slicedFrames, nameBasis + "_slicedFrames_preFF",
                   poolLocation=outputDir)
    saveSlicedCopy(point, nameBasis + "_pointingOffsetProduct",
                   poolLocation=outputDir) 

# Refine the spectral flatfield.
# For very short ranges (<~5 um, when the continuum can be approximated by a
# straight line) consider using the line-scan pipeline script.
# . excludeLeaks=True will remove wavelength ranges affected by spectral 
#   leakages.
...
#   The spectrum outside of the selected range must be discarded from any
#   scientific analysis.
...
slicedFrames = specFlatFieldRange(slicedFrames, polyOrder=3, verbose=verbose,
                                  excludeLeaks=excludeLeaks, selectedRange=None,
                                  useSplinesModel=useSplinesModel)
...
# The wavelength grid is used to perform a final sigma clipping on the spectrum
#     (specFlagOutliers) and then to rebin the spectrum (specWaveRebin)
# Note that for the final cube rebinning it is recommended that: 
#    - For single scan SED or Nyquist sampled range scans, it is recommended to 
#        perform the final rebinning with oversample=2, upsample>=1, which 
#        corresponds to the native resolution of the instrument
#    - For line scan, deep range scans or Nyquist sampled range scans with 
#        repetition factor > 1, oversampling > 2 is made possible by the high
#        degree of redundancy provided by the observation
...
# Rebin all selected cubes on the consistent wavelength grids (mandatory for 
# the next task specAddNod) To compare the rebinned spectrum to the dot cloud
# (possibly with various rebinning parameters), see the PDRG (Chap 3)
...
# This is the final science-grade product currently possible, for all
# observations except the spatially oversampled rasters
slicedFinalCubes = specAddNodCubes(slicedRebinnedCubes)

if verbose: 
    slicedSummary(slicedFinalCubes)
    # plot all slices for a given spaxel (at this stage, normally only 1 /
    # range and / raster position)
    x, y = 2, 2
    p11 = plotCubes(slicedFinalCubes, x=x, y=y)
    # 5x5 plot of one of the rebinned cubes (one line/range at one raster
    # position)
    slice = 0
...
# Before you proceed with the scientific analysis, you will need to make a 
# choice based on the nature of your object (extended or not), the type of
# observation that was performed (pointed or raster mapping), and its spatial
# sampling (oversampled or not). 
...
# the final science-grade product is the "rebinned" cubes, or "interpolated"
# cubes

# ------- POINT SOURCES --------------------------------------------------------

# For single pointing observations of point sources, the calibrated spectrum is
# obtained by correcting for the fraction of the PSF that falls out of the
# (central) spaxel.
# That is what we call the point source correction. This is done in
# extractCentralSpectrum.

# extractCentralSpectrum produces three extra spectra.
# - "c1" is the central spaxel's spectrum with the point source correction
#    applied 
...
#    is more robust against slight pointing offsets (< 1/2 spaxel) and jitter
#    (in range-scans). 
...
# If your source is not in the central spaxel, you cannot use
# "extractCentralSpectrum". Instead, see the dedicated script in menu 'Scripts'.
...
#    1.  "width" (=gaussianFilterWidth) controls the smoothing itself. If 
#    width = 0, a wavelength-independent correction (i.e. a scalar) is applied
#    (one per range)
#    2.  "preFilterWidth" (=medianFilterWidth) is used to avoid any influence 
#    of / on the spectral lines in the correction
# B. Via wavelet decomposition. The intensity of the smoothing is controlled by
#    the parameter nLowFreq, which describes the number of low-frequency layers 
#    to keep from the wavelet decomposition to build the smoothed correction.
...
# too aggressively, or not enough will under/over correct the effect of the 
# pointing jitter on the spectral shape.
# Typical values of 'width' should be of a few tens (e.g. 50), and will depend
# on the sampling of the observation (Nyquist/SED or deep scan), over- and
# up-sampling adopted for the rebinning, the influence of the pointing jitter
# on your peculiar observation, etc.
...
for slice in range(len(slicedFinalCubes.refs)):
    ...
    # c. Scale central spaxel to the level of the central_3x3 (c129 -> See PDRG 
    # & print extractCentralSpectrum.__doc__)
    c1_c5, c9_c5, c129_c5 = extractCentralSpectrum(slicedFinalCubes.get(slice),
                                                   smoothing=smoothing,
    ...
    if saveOutput:
        ...
        simpleFitsWriter(product=c9_c5,file = outputDir + name + str(slice).zfill(2)+".fits")
        name = nameBasis + "_centralSpaxel_PointSourceCorrected_Corrected3x3YES_slice_"
        simpleFitsWriter(product=c129_c5,file = outputDir + name + str(slice).zfill(2)+".fits")
