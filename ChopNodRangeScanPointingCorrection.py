...
# Next, get the data
useHsa = 1
obs_c = getObservation(obsid, verbose=True, useHsa=useHsa, poolLocation=None, poolName=None)
"""
if useHsa:
    saveObservation(obs, poolLocation='/home/sgongar/hcss/workspace/Point-offset-correction/pools', poolName='obs_c')
"""
# Show an overview of the uplink parameters of this observation 
if verbose: obsSummary(obs_c)
...
try:
	target      = str(obs_c.meta["object"].value).replace(" ","").replace(".","").replace("+","plus")
	od          = "OD"+str(int(obs_c.meta["odNumber"].value)).zfill(4)
...
saveOutput = True
outputDir  = str(Configuration.getWorkDir())+"/pacsSpecOutC/"
...
slicedFrames = addUtc(slicedFrames, obs_c.auxiliary.timeCorrelation)
...
slicedFrames = specAddInstantPointing(slicedFrames, obs_c.auxiliary.pointing, calTree = calTree, orbitEphem = obs_c.auxiliary.orbitEphemeris, horizonsProduct = obs_corrrected.auxiliary.horizons)    

# If SSO, move SSO target to a fixed position in sky. This is needed for mapping SSOs.
if (isSolarSystemObject(obs_c)):
  slicedFrames = correctRaDec4Sso (slicedFrames, timeOffset=0, orbitEphem=obs_c.auxiliary.orbitEphemeris, horizonsProduct=obs_c.auxiliary.horizons, linear=0)
...
slicedFrames = specCorrectHerschelVelocity(slicedFrames, obs_c.auxiliary.orbitEphemeris, obs_c.auxiliary.pointing, obs_c.auxiliary.timeCorrelation, obs_c.auxiliary.horizons, force=True)
...
slicedFrames, background = specRespCalToTelescope(slicedFrames, obs_c.auxiliary.hk, calTree = calTree, reduceNoise = 1)
...
for slice in range(len(slicedFinalCubes.refs)):
    ...
    c1_c,c9_c,c129_c = extractCentralSpectrum(slicedFinalCubes.get(slice), smoothing=smoothing, width=gaussianFilterWidth, preFilterWidth=medianFilterWidth, nLowFreq=nLowFreq, calTree=calTree, verbose=verbose)
    #
    # Save to Fits
    if saveOutput:
        ...
        simpleFitsWriter(product=c9_c,file = outputDir + name + str(slice).zfill(2)+".fits")
        name = nameBasis + "_centralSpaxel_PointSourceCorrected_Corrected3x3YES_slice_"
        simpleFitsWriter(product=c129_c,file = outputDir + name + str(slice).zfill(2)+".fits")
